...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='use rnnt to generate alignment')
    ...
    with torch.no_grad(), open(args.result_file, 'w',
        ...
        for batch_idx, batch in enumerate(ali_data_loader):
            ...
            results, rnnt_probs = [], []
            decoded_seq, score_seq = model.rnnt_decode(feat, encoder_out, feats_length)
            results.append(decoded_seq)
            rnnt_probs.append(score_seq)
            rnnt_probs = rnnt_probs.squeeze(0)

            target = target.squeeze(0)
            alignment = forced_align(rnnt_probs, target)
            print(alignment)
